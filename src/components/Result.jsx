...
function Result(props) {
  return (
    ...
    <div className="img-palette-container">
      ...
      <img className="resized-img" src={`data:image/jpeg;base64,${props.data.img_data}`}/>

        <div className="color-palette">
          {props.data.color_palette.map((color, index) => (
            <div key={index} className="color-place" style={{ backgroundColor: color }}></div>
          ))}
        </div>
      </div>
        ...
        <div className="row">
          {props.data.top_20_colors.map((color, index) => (
            <div key={index} className="col-lg-4 col-sm-6 color-box">
              <div className="color-header" style={{ backgroundColor: color.hex }}>
